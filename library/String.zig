...
comptime_created: bool = false,
string: ?[:0]const Char,

pub fn create(str: []const Char) String {
    if (@inComptime()) {
        //In comptime, we can just raw copy the pointer
        return String{
            .comptime_created = true,
            .string = str[0..str.len :0],
        };
    } else {
...
pub fn getSlice(self: String) []const Char {
    return self.string.?;
}

pub fn getSentinal(self: String) [:0]const Char {
    return self.string.?;
}
...
test "create string" {
    ...
    str.destroy();
    try testing.expectEqual(null, str.string);
}
...
test "comptime create" {
    comptime {
        var str = String.create("abc");

        try testing.expect(str.string != null);
        try testing.expectEqual(true, str.comptime_created);
        try testing.expectEqual(3, str.length());
        try testing.expectEqualStrings("abc", str.getSlice());
        str.destroy();
        try testing.expectEqual(null, str.string);
    ...
    var str = comptime String.create("abc");
...
test "comptime concat" {
    comptime {
        ...
        try testing.expect(str_combined.string != null);
        try testing.expectEqual(true, str_combined.comptime_created);
        try testing.expectEqual(6, str_combined.length());
        try testing.expectEqualStrings("abcdef", str_combined.getSlice());
    }
}
